...
tl_h <- tl %>%
  group_by(receiver_name, depth, daily, jdate, year, hours) %>%
...
tl_h <- tl_h %>% 
  ...
  mutate(start_event = if_else(hours %in% ymd_hms(c("2019-05-01 00:00:00",
                                                   "2020-05-01 00:00:00")),
                               true = TRUE, 
                               false = FALSE)) %>% 
  ungroup()
...
tl_h <- tl_h %>% 
  mutate(hour = as.numeric(hours))
# n <- tl_h %>% 
#   filter(daily == "2020-05-01")
# 
# n
# 
# tl_d <- tl_d %>% 
...
rec_num <- unique(tl_h$receiver_name)
...
for (rec in 1:length(rec_num)) {
  ...
  temp_raster_smooth <- crossing(
    tibble(jdate = unique(df$jdate)),
    tibble(hour = unique(df$hour)),
...
temp_merge <- temp_merge %>% 
  mutate(daily = as.Date(hour, origin = "1970-01-01"))
glimpse(temp_merge)
...
#                           "Heatmap-May-Oct_Papineau Lake_GAM_year.png"),
